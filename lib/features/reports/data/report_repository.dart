...
class SqliteReportRepository implements ReportRepository {
  ...
  Future<RangeReport> getRangeReport(DateTime from, DateTime to) async {
    ...
    final orderRows = await database.rawQuery(
      "SELECT order_number, closed_at, status, subtotal, tax_amount, total, COALESCE(payment_method_name, 'Unspecified') AS payment_method FROM orders WHERE status = 'CLOSED' AND deleted_at IS NULL AND closed_at >= ? AND closed_at < ? ORDER BY closed_at",
      args,
    );
